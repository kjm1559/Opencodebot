...
def setup_logging():
    ...
    file_handler = RotatingFileHandler(
        get_next_log_file(), maxBytes=LOG_MAX_BYTES, backupCount=LOG_FILE_COUNT
    )
...
def run_opencode_command(
    ...
    try:
        result = subprocess.run(
            ["opencode"] + args, capture_output=True, text=True, check=True, timeout=timeout
        )
        return result
    ...
    try:
                    ...
                    # Text output - send to Telegram
                        ...
                        if text_content.strip():
                            ...
                            try:
                                bot.send_message(
                                    chat_id,
                                    escape_markdown_v2(display_text),
                                    parse_mode="MarkdownV2",
                                )
                    ...
                    # Session started - show session ID
                        ...
                        if session_id:
                            ...
                            if msg not in sent_messages:
                                ...
                                try:
                                    bot.send_message(
                                        chat_id, escape_markdown_v2(msg), parse_mode="MarkdownV2"
                                    )
        ...
        try:
            ...
            if detail_text and len(detail_text) > 1000:
                ...
                chunk_size = 3500
                chunks = [
                    detail_text[i : i + chunk_size] for i in range(0, len(detail_text), chunk_size)
                ]
        ...
        bot.send_message(
            chat_id, escape_markdown_v2("━─━─━─━─━─━─━─━─\n✅ Completed"), parse_mode="MarkdownV2"
        )
        ...
        bot.send_message(
            chat_id, escape_markdown_v2(f"❌ Error: {str(e)}"), parse_mode="MarkdownV2"
        )
...
def handle_project_command(message):
    ...
    if project_input.startswith(("git@", "http://", "https://", "git://")):
        ...
        try:
            ...
            if result.returncode != 0:
                bot.reply_to(
                    message,
                    escape_markdown_v2(f"❌ Clone failed: {result.stderr}"),
                    parse_mode="MarkdownV2",
                )
                return
            ...
            bot.reply_to(
                message, escape_markdown_v2(f"❌ Error: {str(e)}"), parse_mode="MarkdownV2"
            )
        return
        ...
        if os.path.exists(workspace_dir) and os.path.isdir(workspace_dir):
            for item in os.listdir(workspace_dir):
                ...
                if os.path.isdir(item_path) and (
                    os.path.isfile(git_path) or os.path.isdir(git_path)
                ):
    ...
    if project_input.isdigit():
        ...
        if 0 <= idx < len(workspace_projects):
            ...
            bot.reply_to(
                message, escape_markdown_v2("❌ Invalid project number"), parse_mode="MarkdownV2"
            )
        return
    ...
    if os.path.exists(project_path) and os.path.isdir(project_path):
        ...
        bot.reply_to(
            message, escape_markdown_v2(f"❌ Not found: {project_input}"), parse_mode="MarkdownV2"
        )
...
def handle_project_list_command(message):
    ...
    try:
            ...
            for item in os.listdir(workspace_dir):
                ...
                if os.path.isdir(item_path):
                    ...
                    if os.path.exists(os.path.join(item_path, ".git")):
                        marker = (
                            "✅" if current_project and item_path in str(current_project) else " "
                        )
        ...
        marker = (
            "✅"
            if current_project
            and os.path.abspath(current_project) in os.path.abspath(current_project)
            else " "
        )
        ...
        if project_input:
            if project_input == "root" or project_input == ".":
                ...
                bot.reply_to(
                    message,
                    escape_markdown_v2(
                        f"📁 Set to project root: {os.path.abspath(current_project)}"
                    ),
                    parse_mode="MarkdownV2",
                )
                return
    ...
    try:
        ...
        if not session_id:
            escaped_message = escape_markdown_v2(
                "Please provide a session ID. Usage: /set_session <session_id>"
            )
            bot.reply_to(message, escaped_message, parse_mode="MarkdownV2")
            return
...
def handle_current_session_command(message):
    ...
    try:
        ...
        if not session_id:
            ...
            try:
                ...
                if sessions_data and isinstance(sessions_data, list):
                    latest_session = max(
                        sessions_data, key=lambda x: x.get("updated", x.get("created", 0))
                    )
        ...
        escaped_error = escape_markdown_v2(
            "❌ Failed to query sessions. Please check opencode status."
        )
        bot.reply_to(message, escaped_error, parse_mode="MarkdownV2")
    except Exception as e:
...
def handle_compact_command(message):
    ...
    try:
        ...
        if result.returncode == 0:
            escaped_message = escape_markdown_v2(
                f"✅ Session exported: {session_id[:12]}...\n\n📦 Session data backed up locally."
            )
        else:
            escaped_message = escape_markdown_v2(
                f"⚠️ Export completed with warnings: {session_id[:12]}..."
            )
...
def handle_reset_command(message):
    ...
    try:
        ...
        set_current_session_id(chat_id, "")
        escaped_message = escape_markdown_v2(
            "Session has been reset. A new session will be created for the next command."
        )
        bot.reply_to(message, escaped_message, parse_mode="MarkdownV2")
    ...
    try:
        ...
        if not model_arg:
            ...
            if model_name in available_models:
                ...
                escaped_message = escape_markdown_v2(
                    f"Model '{model_name}' not found. Use /model to list available models."
                )
                bot.reply_to(message, escaped_message, parse_mode="MarkdownV2")
...
def handle_status_command(message):
    ...
    try:
        ...
        response += (
            "🤖 Model: " + (current_model if current_model else "Not set (using default)") + "\n"
        )
        response += "📁 Project: " + (current_project if current_project else "Not set") + "\n"
        response += "🔀 Session: " + (
            current_session if current_session else "None (will create new)"
        )
...
def handle_history_command(message):
    ...
    try:
        ...
        sorted_sessions = sorted(
            sessions, key=lambda x: x.get("updated", x.get("created", 0)), reverse=True
        )
    ...
    try:
        if chat_id in active_process:
            ...
            if process.poll() is None:  # Process still running
                ...
                bot.reply_to(
                    message, "❌ No command is currently running.", parse_mode="MarkdownV2"
                )
...
def handle_restart_command(message):
    ...
    try:
        ...
        try:
            bot.reply_to(
                message,
                escape_markdown_v2("📥 Fetching latest updates via git pull..."),
                parse_mode="MarkdownV2",
            )
            ...
            if git_result.returncode == 0:
                ...
                bot.reply_to(
                    message,
                    escape_markdown_v2(
                        f"⚠️ Git pull completed with warnings: {git_result.stderr[:200]}"
                    ),
                    parse_mode="MarkdownV2",
                )
        except subprocess.TimeoutExpired:
            ...
            bot.reply_to(
                message,
                escape_markdown_v2("⚠️ Git pull timeout, continuing with restart..."),
                parse_mode="MarkdownV2",
            )
            ...
            bot.reply_to(
                message,
                escape_markdown_v2(f"⚠️ Git pull failed: {str(git_error)[:200]}"),
                parse_mode="MarkdownV2",
            )
    ...
    if current_session_id and not force_new_session:
        ...
        try:
            ...
            if sessions_data:
                ...
                logger.info("No existing sessions, creating fresh session")
                command_args = base_args
            ...
            logger.info("Falling back to creating fresh session")
            command_args = base_args
